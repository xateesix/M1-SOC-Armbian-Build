#!/usr/bin/env python3
"""v64: PWM0 lightbar with pwm0-pin pinctrl fix on factory RK3308 DTB.
...
def _fix_pwm0_pinctrl(text: str) -> str:
    """Factory DTB points pwm@ff160000 at pwm8-pin (GPIO2_A10). Use GPIO0_B5."""
    if "pwm0-pin-active" in text:
        return text
    old = "\n\t\t\tpwm0-pin {\n\t\t\t\trockchip,pins = <0x00 0x0d 0x01 0x83>;\n\t\t\t};"
    new = old + "\n\n\t\t\tpwm0-pin-active {\n\t\t\t\trockchip,pins = <0x00 0x0d 0x01 0x83>;\n\t\t\t\tphandle = <0x100>;\n\t\t\t};"
    if old not in text:
        raise RuntimeError("pwm0-pin node not found")
    ...
    inner = re.sub(r"pinctrl-0\s*=\s*<[^>]+>\s*;", "pinctrl-0 = <0x100>;", inner, count=1)
...
def main() -> int:
    ...
    print(f"v64 lights DTB: {args.output} ({args.output.stat().st_size} bytes)")
    return 0
