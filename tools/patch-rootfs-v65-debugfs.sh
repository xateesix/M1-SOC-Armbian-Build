...
for f in lightbar-test.sh neopixel-test.py neopixel-pin-sweep.py fb-color-test.py probe-sdcard.sh factory-led-audit.sh gpio-dmm-probe.sh; do
  [[ -f "$TOOLS/$f" ]] || continue
...
[Service]
Type=oneshot
ExecStart=/bin/sh -c '/sbin/modprobe leds-pwm 2>/dev/null || /sbin/insmod /lib/modules/6.18.0-dirty/kernel/drivers/leds/leds-pwm.ko'
RemainAfterExit=yes
...
if [[ -n "$PS1" && "$(id -u)" -eq 0 ]]; then
  echo "RK3308BS v65: lightbar-test.sh | factory-led-audit.sh | gpio-dmm-probe.sh | probe-sdcard.sh"
fi
EOF
...
debugfs -w -R "mkdir /lib/modules/6.18.0-dirty/kernel/drivers" "$IMG" 2>/dev/null || true
debugfs -w -R "mkdir /lib/modules/6.18.0-dirty/kernel/drivers/leds" "$IMG" 2>/dev/null || true
...
rm -f "$HDR"
# Friendly board name on login status line
REL_PATCH="$STAGE/armbian-release"
debugfs -R "dump /etc/armbian-release $REL_PATCH" "$IMG" 2>/dev/null || true
if [[ -f "$REL_PATCH" ]]; then
  sed -i 's/^BOARD_NAME=.*/BOARD_NAME="A3D M1 Pro X1"/' "$REL_PATCH"
  df_write "$REL_PATCH" /etc/armbian-release
fi
rm -f "$REL_PATCH"

echo "Wrote $OUT via debugfs (v65: lights + MOTD A3D + audit tools)"
