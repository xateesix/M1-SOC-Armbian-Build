...
BUILD_DIR="/tmp/rk3308bs-v24-build"
source "$CONFIG"
echo "=== v24 monolithic release ==="
...
if bash "$TOOLS/install-kernel-modules-wifi.sh" "$BUILD_DIR/rootfs-patched.img" "$BUILD_DIR/rootfs-v24.img"; then
  echo "Modules: loop mount + depmod"
else
  echo "Modules: debugfs fallback"
  bash "$TOOLS/install-kernel-modules-debugfs-wifi.sh" "$BUILD_DIR/rootfs-patched.img" "$BUILD_DIR/rootfs-v24.img"
fi
cp -f "$BUILD_DIR/rootfs-v24.img" "$REL/rootfs-v24.img"
...
powershell.exe -NoProfile -ExecutionPolicy Bypass -File "$(wslpath -w "$SCRIPT_DIR/windows-pack-update.ps1")" \
  -PackInput "$WIN_PACK" -Output "rk3308bs-1.0.0-emmc-fixed-v24.img"
cp -f "$REL/rk3308bs-1.0.0-emmc-fixed-v24.img" "$REL/rk3308bs-1.0.0-emmc-fixed.img"
ls -la "$REL/rk3308bs-1.0.0-emmc-fixed-v24.img"
