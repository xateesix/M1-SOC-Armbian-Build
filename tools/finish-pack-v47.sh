...
powershell.exe -NoProfile -ExecutionPolicy Bypass -File "$(wslpath -w "$SCRIPT_DIR/windows-pack-update.ps1")" \
  -PackInput "$WIN_PACK" -Output "rk3308bs-1.0.0-emmc-fixed-v47.img"
grep CMDLINE "$REL/pack_input_v39/Image/parameter.txt"
ls -la "$REL/rk3308bs-1.0.0-emmc-fixed-v47.img"
cp -f "$REL/rk3308bs-1.0.0-emmc-fixed-v47.img" "$REL/rk3308bs-1.0.0-emmc-fixed.img"
echo "FLASH CHECK: log must show total~1677655552 and rootfs write ~47s (NOT total=30342656)"
echo "DONE: $REL/rk3308bs-1.0.0-emmc-fixed-v47.img"
