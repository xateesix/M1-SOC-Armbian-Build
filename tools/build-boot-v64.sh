#!/usr/bin/env bash
# v64 boot: v59 base + PWM0 lightbar in DTB.
set -euo pipefail
...
BOOTARGS="earlycon=uart8250,mmio32,0xff0d0000 console=tty0 console=ttyS3,1500000n8 systemd.color=never loglevel=7 root=PARTUUID=614e0000-0000-4b53-8000-1d28000054a9 rootfstype=ext4 rw rootwait"
...
dtc -I dtb -O dts "$REL/_fac-dtb-v64.dtb" 2>/dev/null | grep -q 'compatible = "simple-panel"' \
  || { echo "FAIL: expected simple-panel in v64 DTB"; exit 1; }
...
echo "boot-v64 ok (simple-panel + pwm0 lightbar DTB)"
