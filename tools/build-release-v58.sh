#!/usr/bin/env bash
# v58: v57 rootfs display fixes + v36-style factory boot (simple-panel + factory logo).
set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
REL="$SCRIPT_DIR/releases/1.0.0"
TOOLS="$SCRIPT_DIR/tools"
BUILD_DIR="$SCRIPT_DIR/.build-v58"
TMPDIR="$SCRIPT_DIR/.wsl-tmp"
export TMPDIR
mkdir -p "$TMPDIR" "$BUILD_DIR"
export RK3308BS_IMAGE_TAG="v58-factory-display"
...
bash "$TOOLS/install-kernel-modules-chroot-display.sh" "$BUILD_DIR/rootfs-expanded.img" "$BUILD_DIR/rootfs-v58.img"
cp -f "$BUILD_DIR/rootfs-v58.img" "$REL/rootfs-v58.img"

bash "$TOOLS/build-boot-v58.sh" "$REL/_Image-v22"
...
cp "$REL/rootfs-v58.img" "$PACK/Image/rootfs.img"
...
powershell.exe -NoProfile -ExecutionPolicy Bypass -File "$(wslpath -w "$SCRIPT_DIR/windows-pack-update.ps1")" \
  -PackInput "$WIN_PACK" -Output "rk3308bs-1.0.0-emmc-fixed-v58.img"
cp -f "$REL/rk3308bs-1.0.0-emmc-fixed-v58.img" "$REL/rk3308bs-1.0.0-emmc-fixed.img"
ls -la "$REL/rk3308bs-1.0.0-emmc-fixed-v58.img"
echo "DONE: $REL/rk3308bs-1.0.0-emmc-fixed-v58.img"
