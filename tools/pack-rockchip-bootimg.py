#!/usr/bin/env python3
"""Assemble Rockchip Android boot.img (LZ4 kernel + resource.img second slot)."""
...
def build_bootimg(
    ...
    header[:8] = b"ANDROID!"
    # Match factory Rockchip layout: resource.img lives in the *second* slot, not ramdisk.
    struct.pack_into(
        "<8I",
        header,
        8,
        ks,  # kernel_size
        0x10008000,  # kernel_addr (factory boot.img)
        rs,  # ramdisk_size (0)
        0,  # ramdisk_addr
        ss,  # second_size (resource.img)
        0x10F00000,  # second_addr (factory boot.img)
        0x10000100,  # tags_addr (factory boot.img)
        page_size,
    )
    header[0x50 : 0x50 + len(cmdline_bytes)] = cmdline_bytes

    kernel_off = page_size
    second_off = page_size + page_align(ks, page_size)
...
def main() -> int:
    ...
    boot = build_bootimg(kernel, second, args.cmdline)
    args.output.write_bytes(boot)
    print(f"Wrote {args.output} ({len(boot)} bytes)")
    print(f"  kernel_size={len(kernel)} second_size={len(second)}")
    return 0
